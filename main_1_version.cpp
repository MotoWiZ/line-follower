...
uint16_t valVolt, sensor[5], refTracker[10]={}, motorSpeed1, motorSpeed2, speed=350, FlipFlop=500;
uint32_t normSensor[5];
float volt, motorSpeed, position, P, I, D, lastP, kP=0.06, kI=0.04, kD=0.003;
unsigned long mils=0, curmils=0;
...
void loop() 
{
    ...
    if (start == 1)
    {
        if ((millis() - mils) >= PERIOD)
        {
            ...
            motorSpeed1 = speed + motorSpeed;
            motorSpeed2 = speed - motorSpeed;           
        }
    }
    voltage();
    motor();
